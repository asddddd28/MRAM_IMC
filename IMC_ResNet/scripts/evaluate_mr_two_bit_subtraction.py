...
def main():
    ...
    from IMC_ResNet.models.mr_two_bit_match import TwoBitMatch
    windows = {stage: {scope: TwoBitMatch(planes) for scope, planes in [('within_b4',[3,2,1]),('include_mr0_b5',[3,2,1,0])]}
               for stage in ['before_carry','after_carry_before_any_cancel','after_current_b0_b1_b2']}
    ...
    with torch.no_grad():
        for batch, (x, _) in enumerate(loader):
            ...
            for t in range(64):
                ...
                before = trace['before_b0_interception']
                raw = before.clone()
                raw[...,4] += trace['incoming_negative_units']
                for scope in windows['before_carry']:
                    windows['before_carry'][scope].observe(before)
                    windows['after_carry_before_any_cancel'][scope].observe(raw)
                ...
                for stats in windows['after_current_b0_b1_b2'].values():
    ...
    report['two_bit_full_subtraction_opportunities'] = {stage: {scope:s.report() for scope,s in values.items()} for stage,values in windows.items()}
    assert opportunities[0].matched == b0_pairs
    assert opportunities[1].matched == b1_pairs
    assert opportunities[2].matched == b2_pairs
    out = ROOT/'IMC_ResNet/checkpoints/mr_two_bit_subtraction_opportunities.json'
